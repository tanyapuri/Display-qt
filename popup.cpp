...
void popup::popup_show()
{
    ...
    horizontitle << "date" << "time" << "types" << "information";
    ...
    for(int i=0;i<size;i++){
        ...
        QString type_q="";
        if(date_time[i][5]==0) type_q="daily";
        if(date_time[i][5]==1) type_q="learn";
        if(date_time[i][5]==2) type_q="entertainment";
        if(date_time[i][5]==3) type_q="other";
...
void popup::tv_show_p()
{
    ...
    QStringList horizontitle;
    horizontitle << "Play title" << "Number of Seasons" << "Must Watch Episodes" ;
