    ...
    ui(new Ui::memo_save)
{
    ...
    ui->actiontype_richang->setText(tr("daily"));
    ui->actiontype_xuexi->setText(tr("learn"));
    ui->actiontype_yule->setText(tr("entertainment"));
    ui->actiontype_qita->setText(tr("other"));

    ui->pushButton_type->setMenu(menu_type);

    ui->pushButton_type->setText(tr("choose a type"));
    ...
    ui->lineEdit_info->setText("Please enter information");
    ui->textEdit_info->setText("Please enter information");
}
...
void memo_save::onMenu_1_Triggered(QAction * action)
{
    if(action==ui->actiontype_richang){
        ui->pushButton_type->setText(tr("learn"));
        type=0;
    }
    else if(action==ui->actiontype_xuexi){
        ui->pushButton_type->setText(tr("daily"));
        type=1;
    }
    else if(action==ui->actiontype_yule){
        ui->pushButton_type->setText(tr("entertainment"));
        type=2;
    }
    else if(action==ui->actiontype_qita){
        ui->pushButton_type->setText(tr("other"));
        type=3;
    }
}
...
void memo_save::on_buttonBox_accepted()
{
    if(year==0 && month==0 && day==0 && hour==0 && minute==0 && ui->lineEdit_info->text()=="" && ui->textEdit_info->toPlainText()==""){
        QMessageBox::about(this,"warning","Incomplete Information\0");
        return;
    }
    if(day>JulDate::dayofmonth(year,month)){
        QMessageBox::about(this,"warning","Wrong data input");
        return;
    }
